 ...
 *
 * This script syncs the @patro-io/cms version across the monorepo:
 * 1. Reads the version from packages/core/package.json
 * 2. Updates the version in packages/create-app/src/cli.js
 * 3. Syncs the @patro-io/create-cms package version to match core version
 */
...
async function syncVersions() {
  try {
    ...
    const versionRegex = /'@patro-io\/cms': '\^[\d.]+(-[\w.]+)?'/
    const newVersionString = `'@patro-io/cms': '^${coreVersion}'`
    ...
    if (versionRegex.test(cliContent)) {
      ...
      console.log(`✓ Updated create-app CLI to use @patro-io/cms@^${coreVersion}`)
    } else {
      console.warn('⚠ Could not find version string in CLI file to update')
    }

    // Sync @patro-io/create-cms version to match core version
    ...
    console.log(`✓ Synced @patro-io/create-cms from ${oldCreateAppVersion} to ${coreVersion}`)
