...
This document tracks the gradual migration of routes from the PatroCMS monolith to the `@patro-io/cms` package.
...
- Updated imports to use `@patro-io/cms` types
...
- Optional: Custom bucket name (`BUCKET_NAME` env var, defaults to 'patro-media-dev')
...
### Circular Import
Some monolith routes import from `@patro-io/cms` while being part of it. Solution: Use relative imports for internal dependencies.
